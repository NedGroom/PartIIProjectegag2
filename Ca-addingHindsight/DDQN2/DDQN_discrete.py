...
class Memory:
    def __init__(self, len):
        ...
        self.achievedgoals = collections.deque(maxlen=len)
        self.is_done = collections.deque(maxlen=len)

    def update(self, state, action, reward, done, achievedgoal):
        ...
        self.achievedgoals.append(achievedgoal)
...
def evaluate(Qmodel, env, repeats, desiredgoal):
    """
...
def replayMemory(memory, newgoal, episodeLength, tolerance):
    ...
    states = np.array(memory.state)[-episodeLength:]
    actions = np.array(memory.action)[-episodeLength:]
 #   rewards = torch.Tensor(memory.reward)[-episodeLength:]
    achievedgoals = np.array(memory.achievedgoals)[-episodeLength:]
    is_dones = np.array(memory.is_done)[-episodeLength:]

#    state = np.append(state, desiredgoal)

    for idx, state in enumerate(states):
        state = np.append(state[:4],newgoal)
        achievedgoal = achievedgoals[idx]
        done = is_dones[idx]
        if done:
        ...
        memory.action.append(actions[idx])
        memory.rewards.append(reward)
        memory.achievedgoals.append(achievedgoal)
        memory.is_done.append(done)


    #for i in range(-episodeLength, 0):
  ...
  #  env = wrapperDiscrete_newRewardNoHindsight(env, tolerance=tolerance)
    ...
    for episode in range(num_episodes):
        ...
        if episode % measure_step == 0:
            performance.append([episode, evaluate(Q_1, env, measure_repeats, desiredgoal)])
        ...
        while not done and i < horizon:
            ...
            memory.update(state, action, reward, done, achievedgoal)

        done = True

        replayMemory(memory, achievedgoal, i, tolerance)
