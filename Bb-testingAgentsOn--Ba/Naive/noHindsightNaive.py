...
class OLDwrapperPartial_newRewardNoHindsight(gym.Wrapper):
    ...
    for i in range(episodes):
        ...
        while True:
            ...
            if(SoC < wrapped_env.cfg.paper_battery_capacity):
                if (pv-load>0): 
                    action = np.array([(pv-load)/wrapped_env.cfg.paper_max_charge_power],dtype="float32") # divide to make it a relative power
                else: 
...
if __name__ == "__main__":
    ...
    main(episodes=2000, saveload='runNaiveA',num_sample_eps=6, loadscaling=5, seed=1, measure_step=100)
    main(episodes=2000, saveload='runNaiveA',num_sample_eps=6, loadscaling=5, seed=2, measure_step=100)