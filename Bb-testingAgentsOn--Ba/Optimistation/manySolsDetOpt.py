...
### this file allows you to return many solutions to an optimisation. Only safely works for one episode.
...
def loadDataRunOpts(loadscale=3000,solarscale=3000,seed=1000,episodes=3, run=0):
    ...
    path = 'Utils/TestData/exampleTestData-runTest{}/loadscale{}solarscale{}seed{}wrappedTrueeps1'.format(
                                            run,loadscale,solarscale,seed)
    ...
    print("episode start times: ",len(tarr_))
    ...
    #  found=False
    #while not found:
    solutions = runOptimisation(steps
                            , pvs[start:end]
                            , loads[start:end]
                            , SOCarr[idx]/16000)
        # found = len(solutions) > 1

  #  print("data in solutions")
  #  print([data for data in solutions])
#    assert False
    ...
    for data in solutions:
        ...
        totalsteps, totaltimesofday, totalloads, totalpvs = np.array([]), np.array([]), np.array([]), np.array([])
        totalEVPVs, totalEVGs, totalSOCs = np.array([]), np.array([]), np.array([])


    #      print(data["steps"])
    #    assert False
        totaltimesofday = np.append(totaltimesofday,times[start:end]%24) 
        ...
        solutionsdatas.append(totaldata)

    path = 'output/Data/1eps/detopt2sols-run{}.pkl'.format(run)
    with open(path, 'wb') as fp:
        pickle.dump(solutionsdatas, fp)
...
loadDataRunOpts(episodes=1, seed=1001, run=4)
