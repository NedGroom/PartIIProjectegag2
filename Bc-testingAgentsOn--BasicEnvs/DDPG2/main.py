...
def train(num_episodes, agent, env,  evaluate, validate_every, output, max_episode_length=None, 
          num_sample_eps=30, debug=True, warmup=0, bsize=2, epsilon=2, seed=2, loadscaling=None):
    ...
    #    assert(env.time_step == env.load.time_step%24)
        ...
        if episode > warmup :
            ...
            if episode % validate_every == 0 and validate_every > 0 and countsampleepisodes < numsampleepisodes:
                eptimes = np.append(eptimes,episode_steps)
  ...
  #          agent.save_model(output) ###was about to add a if correct episode store display data
        ...
        if done: # end of episode
            ...
            print("new episode" + str(num_episodes - episode))
            agent.memory.append(
                observation,
     ...
     #       print(info)
      ...
      #      totalcosts.append(info["total_cost"])
       ...
       #         dataslice["socs"] = socs
                ...
                dataslice["step_at_storing"] = step
...
def save_results_with_data(testrewards, dataslices, namecfg, fn, interval=None):
    ...
    #    errorcosts = [np.std(slice["costs"]/maxcost) for slice in dataslices]  
        ...
        xstep = [slice["step_at_storing"] for slice in dataslices]
        ...
        ax[0].errorbar(x, yrew, fmt='-ko')
        ax[0].errorbar(x, yrewards, fmt='-ro')

        ax[1].errorbar(xstep, yrew, fmt='-ko')
        ax[1].errorbar(xstep, yrewards, fmt='-ro')
     ...
     #   ax.errorbar(x, ymaxpvs, yerr=errormaxpv, fmt='c.')
        ...
        ax[0].legend(['test av reward','interval av reward']) #,'interval av total costs', 'interval av SoCs','interval av pv consumption','interval av max pv consum'])
...
def plotsampleepisodeslong(data, path, loadscaling):
    ...
    for ep in range(numeps):
     ...
     #   timesteps = timesteps + add
        axarr[plotindex].plot(timesteps, rewards)    # this will call an error if train eps too small, or validateevery too big
    ...
    #    axarr[axidaplotindex].axvspan(20, 31, alpha=0.25, color='grey')
...
def main(mode='', train_eps=0, bsize=64, epsilon=50000, validate_eps=20, validate_every=1000, seed=1, warmup=0, saveload='default', loadscaling=None):
    ...
    parser.add_argument('--max_episode_length', default=2500, type=int, help='')
...
#    env = wrapperPartial_newRewardNoHindsight(env)
  ...
  #      env.seed(args.seed)
   ...
   # num_states = len(env.observation_space)
    ...
    evaluate = Evaluator(args.validate_episodes, interval=args.validate_every, save_path=args.saveload, args=args, max_episode_length=args.max_episode_length)


    if args.mode == 'train':
        train(args.train_iter, agent, env, evaluate, args.validate_every, args.saveload, 
              warmup=args.warmup, num_sample_eps = num_sample_eps, debug=args.debug, bsize=bsize, epsilon=epsilon, max_episode_length=args.max_episode_length, seed = args.seed, loadscaling=loadscaling)
