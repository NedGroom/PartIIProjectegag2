...
class wrapperPara_newRewardNoHindsight(gym.Wrapper):
    ...
    #    action = np.float32(action) # Is this meant to be here?
     ...
     #   print((pv_generation - solar_used)[0])
        assert ((pv_generation - solar_used)[0]  >= -0.000005 * self.cfg.solar_scaling_factor)
        cum_load += load
