...
class wrapperPartial_newRewardNoHindsight(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if (terminated):
            ...
            self.episode += 1
            self.my_pv_consumption = 100 * self.state["cum_pv_used"] / self.state["cum_pv_gen"]
            cum_EVmaxPV = min(self.state["cum_EVmaxPV"], self.cfg.paper_battery_capacity - self.SoC_on_arrival)
            self.max_pv_consumption = 100 * (self.state["cum_LmaxPV"] + cum_EVmaxPV) / self.state["cum_pv_gen"]
            self.total_cost = self.state["cum_cost"]
        else:
            reward = 0
    ...
    def reset(        
        ...
        *,
        ...
        if hasattr(self.cfg,"_np_random"):
            nprandom = self.cfg._np_random
        else:
            nprandom=None
        time = self.time_step
        ...
        if seed is not None:
            self.cfg._np_random, seed = gym.utils.seeding.np_random(seed)
